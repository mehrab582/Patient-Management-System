<?php 
	...
	if(!empty($_POST["patid"])&&!empty($_POST["docid"])) {
		...
		if($find_rslt->num_rows < 20) {
			...
			header("location:patient_createap.php?smsg=".urlencode("Successfully Inserted Appointment !"));
		} else {
			header("location:patient_createap.php?emsg=".urlencode("Cannot make more than 20 appointments !"));
		}
	}
 ?>
